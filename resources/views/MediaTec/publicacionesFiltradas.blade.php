                            ...
                            @foreach($publicaciones as $publicacion)
                                <div class="col-lg-4 col-md-6 col-sm-6">
                                    <article class="post-grid mb-5 ">
                                        <a class="post-thumb mb-4 d-block" href="/nota/{{$publicacion->publicacion_id}}">
                                             
                                        @switch($publicacion->tipo)
                                                @case("imagen")
                                                    <img class="img-fluid mx-auto mb-4" src="{{asset($publicacion->ruta)}}" style="width: 400px; height: 300px;">
                                                    @break
                                                @case("video")
                                                    <video src="{{asset($publicacion->ruta)}}" width="400px" height="300px"></video>
                                                    @break
                                                @case("audio")
                                                    <audio src="{{asset($publicacion->ruta)}}" width="1500px" height="1500px" controls></audio>
                                                    @break
                                                @case("documento")
                                        ...
                                        <div class="post-content-grid">
                                            ...
                                            <h3 class="post-title mt-1"><a href="/nota/{{$publicacion->publicacion_id}}">{{$publicacion->titulo}}</a></h3>
